...
%\item \path{msword/}: contains the MS word signatures pages and an example template for the various degrees.
%  These templates are automatically generated from the \LaTeX{} templates on occasion and are only for reference.
%\item \path{latex/}: containes LaTeX templates
%\item \path{deadlines.xlsx}: A deadline calculator that uses the semester's
%graduation date
%\item \path{RU-TVD-MSC-rules-2017.pdf}: RU Master's Thesis rules and information
\end{itemize}
...
Many notes are at the top of the \path{DEGREE-NAME-YEAR.tex} file, this file
